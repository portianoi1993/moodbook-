...
const PROFILES = [
  ...
  { key: 'fantasy', match: /fantasy|dragon|magic|wizard|witch|kingdom|quest|sword|\belf\b|\belves\b|\bfae\b|throne|court of|\bwings?\b/i,
  ...
  { key: 'scifi', match: /science[- ]?fiction|sci-?fi|\bspace|galaxy|planet|robot|android|cyber|dystopi|future|\bmars\b|\bstars?\b|starship|interstellar/i,
  ...
  { key: 'romance', match: /romance|romantic|\blove|wedding|heart|\bkiss|bride|affair|passion|boyfriend|girlfriend/i,
  ...
  { key: 'historical', match: /historical|\bwar\b|century|victorian|regency|empire|\bkings?\b|\bqueens?\b|revolution|ancient|greek|\broman\b|\bmyth/i,
...
export function composeOffline({ title, author, genre, desc, mood, style, sound }) {
  ...
  // Fiction genres first: the non-fiction profile has broad words (health, money, success…) that also
  // show up in novel descriptions, so it only wins when no fiction genre matched.
  const prof = PROFILES.find((p) => p.key !== 'selfhelp' && p.match.test(hay)) || PROFILES.find((p) => p.match.test(hay)) || DEFAULT;
