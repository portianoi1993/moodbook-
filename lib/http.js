...
// Optional shared counter (set by lib/store.js at import time to avoid a circular import).
let sharedIncr = null;
export function useSharedCounter(fn) { sharedIncr = fn; }

/**
 * Guard: rate limit + method check. Returns true when the request was rejected.
 * With a shared store configured the limit is enforced across every instance; otherwise per instance.
 */
export async function guard(req, res, { methods = ['GET'], max = 30, windowMs = 60 * 60 * 1000 } = {}) {
  ...
  const id = `${scope}|${clientIp(req)}`;
  let lim = rateLimit(id, { max, windowMs });
  if (sharedIncr) {
    const slot = Math.floor(Date.now() / windowMs);
    const n = await sharedIncr(`mb:rl:${id}:${slot}`, windowMs / 1000);
    if (typeof n === 'number') lim = { ok: n <= max, remaining: Math.max(0, max - n), retryAfter: Math.ceil(((slot + 1) * windowMs - Date.now()) / 1000) };
  }
